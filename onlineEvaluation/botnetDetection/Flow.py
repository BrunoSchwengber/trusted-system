...
bots_str=['147.32.84.165', '147.32.84.191', '147.32.84.192', '147.32.84.193',
...
for bot in bots_str:
	bots.append(socket.inet_aton(bot))
...
def writeFlowsToList(flowlist):

	#flowFeatures = pd.DataFrame(columns=['src', 'dst', 'n_packet1', 'n_byte1', 't_start1',
	#	't_end1', 'InterArrivaltime1', 'n_packet2', 'n_byte2', 't_start2', 't_end2', 'InterArrivaltime2', 'label'])
	#flowFeatures = pd.DataFrame(columns=['src', 'dst', 'n_packet1', 'n_byte1', 't_start1',
	#	't_end1', 'n_packet2', 'n_byte2', 't_start2', 't_end2', 'label'])
	flowFeatures = []
	if flowlist !=None:
		for flow in flowlist:

			dic= pd.DataFrame(data={'src':[socket.inet_ntoa(flow.ip1)],
				'dst':[socket.inet_ntoa(flow.ip2)],
				'n_packet1':[flow.n_packet1],
				'n_byte1':[flow.n_byte1],
				't_start1':[float(flow.t_start1)],
				't_end1':[float(flow.t_end1)],
				#'InterArrival1':'%.6f'%flow.getInterArrivaltime1(),
				'n_packet2':[flow.n_packet2],
				'n_byte2':[flow.n_byte2],
				't_start2':[float(flow.t_start2)],
				't_end2':[float(flow.t_end2)],
				#'InterArrival2':'%.6f'%flow.getInterArrivaltime2(),
				'label':[1 if flow.ip1 in bots else 0]})
			print(flow.ip1 in bots)
			flowFeatures.append(dic)
		#print(flowFeatures)
	return pd.concat(flowFeatures)
