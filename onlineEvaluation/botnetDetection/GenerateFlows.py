...
def generateFlow(pkts):
	...
	for eachpkt in pkts:
		#print(float(eachpkt[IP].time))
		fields = [eachpkt[IP].src, eachpkt[IP].dst, eachpkt.time, eachpkt.len]
		#print(fields)
		packetlist.append(PacketObject(fields))


	outflowlist = packetsToFlows(packetlist, 60)
	#print(outflowlist)
	if outflowlist != None:
		print('flows in '+ str(len(outflowlist)))
	
	flowFeatures = writeFlowsToList(outflowlist)
	print(flowFeatures)
	return flowFeatures
