...
rec1 = plt.bar(r1, am, color='r', width=barwidth, label='Acurácia', yerr=ad)
rec2 = plt.bar(r2, pm, color='blue', width=barwidth, label='Precisão', yerr=ppd)
rec3 = plt.bar(r3, rm, color='g', width=barwidth, label='Recall', yerr=rd)
rec4 = plt.bar(r4, fm, color='orange', width=barwidth, label='F1-Score', yerr=fd)


plt.xlabel('Sistemas')
plt.xticks([r + ((3*barwidth)/2) for r in range(2)], ['TRUSTED\nc/ HAC', 'TRUSTED\nc/ ARF', ])#'FIXED\nw/ HAC', 'TRUSTED\nw/ ARF'])
plt.ylabel('')
