...
z = np.linspace(0, 100, len(window_average(arf.iloc[:,0], 1)), endpoint=True)
...
plt.plot(z, window_average(arf.iloc[:,0], 1), 'r', linestyle='--', label='TRUSTED c/ ARF', marker=".")
#plt.plot(y, window_average(fixo.iloc[:,0], int(len(fixo.iloc[:,0])/30)), 'blue', linestyle='-.', label='FIXED w/ HAC', marker="s") 
plt.plot(x, window_average(trusted.iloc[:,0], int(len(trusted.iloc[:,0])/30)), 'g', linestyle='-', label='TRUSTED c/ HAC', marker="v") 
